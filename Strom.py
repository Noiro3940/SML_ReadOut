...
def ReadSensor():
	global USBSensor
	USBSensor = SML.Sensor('/dev/ttyUSB0',9600)
	Daten = USBSensor.ReadDatenFromSensor()
	return Daten

def SML_Parser(SMLDaten,Filename):
	
	ObisList = [[b'\x07',b'\x01',b'\x00',b'\x01',b'\x08',b'\x00',b'\xFF'],#'OBIS Kennzahl für Zählerstand Wirkenergie Bezug Tarif1',
	...
	ObisDic = {1:'Zählerstand Wirkenergie Bezug Tarif1',
				2:'Gerätenummer = ServerID',
				3:'Hersteller-Bezeichnung',
				4:'Zählerstand Wirkenergie Bezug Tarif2'
	}
	...
	for Entry in ObisList:
		...
		if ObisEntry == 1:
			...
			SaveData(Filename,Zählerstand[0]+','+Zählerstand[1])
		...
		if (ViewAreaData == True) and (SMLEntry != b'\x77'):
			...
			if (ColCounter == 0):
				if RowCounter == 5:#in allen bereichen befindet sich der benötigt wert in der 5 Reihe (eigentlich 6 ;o)   )
					#print(ObisDic[ObisEntry],':',AreaData)
					return AreaData
...
def SaveData(Filename,Zählerstand):
	'''
	...
	if TimeChange:#wenn ein neuer Tag anfängt soll auch eine neue Logfile erzeugt werden.
		Filename = 'TagesStromverbrauch_'+Datum+'.csv'
	else:
		Filename = Filename

	Logfile = open(Filename,'a')
	Logfile.write(Datum+'|'+Zeit+'|'+Zählerstand+'\n')
	Logfile.close()	
